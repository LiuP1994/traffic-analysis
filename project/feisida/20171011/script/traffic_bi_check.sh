...
select terminal,count(username) as cnt from (
select terminal,username from traffic_bi_id_catch_${filedate} where terminal_name = '' and terminal <> ''
group by terminal,username
) t1 group by terminal
) t2 full outer join traffic_bi_check_terminal_${filedate} t3
on t2.terminal = t3.terminal
) t3 group by terminal,cnt;
