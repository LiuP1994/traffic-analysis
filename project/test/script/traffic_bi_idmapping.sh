    ...
    if [[ $# = 1 ]]; then
        ...
        if [[ ${run_type} -eq 0 ]]; then
            ...
            if [[ ${retry_times} -lt ${retry} ]]; then
                ...
                kill_pid traffic_bi_idmapping.sh
            fi
        fi
    fi
}
...
CREATE TABLE if not exists traffic_bi_idmapping (
username string,
tid string,
uid string,
...
STORED AS TEXTFILE
location '${hive_location}/traffic_bi_idmapping';
EOF
`"
...
drop table traffic_bi_idmapping_temp_${filedate};
create table traffic_bi_idmapping_temp_${filedate} (
username string,
tid string,
uid string,
terminal_name string,
...
row format delimited
fields terminated by '\t'
location '${hive_location}/traffic_bi_idmapping_temp_${filedate}';

insert overwrite table traffic_bi_idmapping_temp_${filedate}
select username,tid,uid,terminal_name,unix_timestamp('${filedate} 00:00:00','yyyyMMdd HH:mm:ss') as firstdate,
unix_timestamp('${filedate} 00:00:00','yyyyMMdd HH:mm:ss') as lastdate,1 as dates
from traffic_bi_id_catch_${filedate}_Id2Uid;
EOF
`"
...
insert overwrite table traffic_bi_idmapping partition (dir='${filedate}')
select username,tid,uid,terminal_name,firstdate,lastdate,dates from (
select if(t1.username is null,t2.username,t1.username) as username,
if(t1.tid is null,t2.tid,t1.tid) as tid,
if(t1.uid is null,t2.uid,t1.uid) as uid,
case
...
select username,tid,uid,terminal_name,firstdate,lastdate,dates from traffic_bi_idmapping where dir='${beforedate}'
) t1
full outer join (
select username,tid,uid,terminal_name,firstdate,lastdate,dates from traffic_bi_idmapping_temp_${filedate}
) t2
on t1.username = t2.username and t1.tid = t2.tid and t1.uid = t2.uid
) a group by username,tid,uid,terminal_name,firstdate,lastdate,dates;
EOF
`"
...
drop table traffic_bi_idmapping_temp_${filedate};
alter table traffic_bi_idmapping drop partition (dir='${deldate}');
exit;
EOF
`"