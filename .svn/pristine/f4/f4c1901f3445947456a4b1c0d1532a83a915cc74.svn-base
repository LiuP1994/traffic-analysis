...
select username,url,host,ua,cookie,stattime from (
select username,referer as url,uri as host,cookie as ua,taobao as cookie,stattime as stattime
from etl.r_shkd_userinfo
where dir='${filedate}'
and referer not rlike '(^.*.jpg$)|(^.*.jpeg$)|(^.*.png$)|(^.*.bmp$)|(^.*.gif$)|(^.*.hs$)|(^.*.js$)|(
^.*.css$)'
) t2
) t3  where  t3.ua not rlike '(^.*.Windows.*$)|(^.*windows.*$)|(^.*MSIE.*$)|(^.*Msie.*$)|(^.*msie.*$
)|(^.*Macintosh.*$)|(^.*macintosh.*$)|(^.*dows NT.*$)';
exit;
EOF
`"
