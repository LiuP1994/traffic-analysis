    ...
    if [[ $# = 1 ]]; then
        ...
        if [[ ${run_type} -eq 0 ]]; then
            ...
            if [[ ${retry_times} -lt ${retry} ]]; then
                ...
                kill_pid traffic_bi_terminal.sh
            fi
        fi
    fi
}
...
CREATE TABLE if not exists traffic_bi_terminal (
username string,
tid string,
...
STORED AS TEXTFILE
location '${hive_location}/traffic_bi_terminal';
EOF
`"
...
drop table traffic_bi_terminal_temp_${filedate};
create table traffic_bi_terminal_temp_${filedate} (
username string,
tid string,
terminal_name string,
firstdate bigint,
...
row format delimited
fields terminated by '\t'
location '${hive_location}/traffic_bi_terminal_temp_${filedate}';

insert overwrite table traffic_bi_terminal_temp_${filedate}
select username,tid,terminal_name,unix_timestamp('${filedate} 00:00:00','yyyyMMdd HH:mm:ss') as firstdate,
unix_timestamp('${filedate} 00:00:00','yyyyMMdd HH:mm:ss') as lastdate,1 as dates
from traffic_bi_id_catch_${filedate}_id2terminal;
EOF
`"
...
insert overwrite table traffic_bi_terminal partition (dir='${filedate}')
...
select username,tid,terminal_name,firstdate,lastdate,dates from traffic_bi_terminal where dir='${beforedate}'
) t1
full outer join (
select username,tid,terminal_name,firstdate,lastdate,dates from traffic_bi_terminal_temp_${filedate}
) t2
on t1.username = t2.username and t1.tid = t2.tid
) a group by username,tid,terminal_name,firstdate,lastdate,dates;
EOF
`"
...
drop table traffic_bi_terminal_temp_${filedate};
alter table traffic_bi_terminal drop partition (dir='${deldate}');
exit;
EOF
`"