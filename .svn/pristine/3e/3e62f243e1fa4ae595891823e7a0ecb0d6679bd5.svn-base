...
select username,url,host,ua,cookie,stattime from (
select userip as username,url as url,parse_url(concat('http://',url),'HOST') as host,ua as ua,cookie as cookie,ms_time as stattime
from httpfilter.t_data_wuhan_httpfilter
where dir='${filedate}'
and url <> ''
and url not rlike '(^.*.jpg$)|(^.*.jpeg$)|(^.*.png$)|(^.*.bmp$)|(^.*.gif$)|(^.*.hs$)|(^.*.js$)|(^.*.css$)'
) t2
) t3  where  t3.ua not rlike '(^.*.Windows.*$)|(^.*windows.*$)|(^.*MSIE.*$)|(^.*Msie.*$)|(^.*msie.*$)|(^.*Macintosh.*$)|(^.*macintosh.*$)|(^.*dows NT.*$)';
exit;
EOF
`"
